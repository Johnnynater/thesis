...
def match_distribution(nr_sim, probs):
    ...
    cat = [jensenshannon(probs, np.random.dirichlet(np.ones(len(probs)))) for _ in range(nr_sim)]
    med_cat = np.median(cat)
    ...
    print(np.median(ordinal), med_cat)
    if np.median(ordinal) < med_cat:
        return 'Ordinal'
    else:
        return 'Categorical'
...
def run_inference(df):
    ...
    distribution = match_distribution(1000, probs)
